...
jQuery(document).ready(function ($) {
	...
	function initTour() {
		...
		tourStepInfo.on('click', '.cd-next', function (event) {
			...
			(!$(this).hasClass('inactive')) && changeStep(tourSteps, coverLayer, 'next');
		});
	...
	function createNavigation(steps, n) {
		var tourNavigationHtml = '<div class="cd-nav"><span><b class="cd-actual-step">1</b> из ' + n + '</span><ul class="cd-tour-nav"><li><a href="#0" class="cd-prev"><img src="./img/ledovoe/left.svg" alt=""></a></li><li><a href="#0" class="cd-next"><img src="./img/ledovoe/right.svg" alt=""></a></li></ul></div><a href="#0" class="cd-close">Закрыть</a>';
	...
	function closeTour(steps, wrapper, layer) {
		...
		layer.removeClass('is-visible');
		// $('.map').addClass('map_hover');
		// $('.cd-btn').removeClass('display_none')
		// $('#cd-tour-trigger').html('Пройти еще раз')
	}
