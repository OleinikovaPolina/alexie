...
jQuery(document).ready(function ($) {
  ...
  var halfWindowH = $(window).height() * 0.5,
    ...
    maxRotationY = 5,
    maxRotationX = 3,
    aspectRatio; //detect if hero <img> has been loaded and evaluate its aspect-ratio
  ...
  $(window).on('resize', function () {
    if ($('html').hasClass('preserve-3d')) {
      window.requestAnimationFrame(function () {
        halfWindowH = $(window).height() * 0.5, halfWindowW = $(window).width() * 0.5;
        initBackground();
      });
    } else {
  ...
  function initBackground() {
    ...
    var wrapperHeight = Math.ceil(halfWindowW * 2 / aspectRatio),
      ...
      newTop = (wrapperHeight - newImageHeight) / 2; //set an height for the .cd-background-wrapper
  ...
  var number = 0;
  var lastnumber = 0;
  var newlength = 0; //cache DOM elements
    ...
    $(this).on('click', function (event) {
      if (animating == false) {
        ...
        if (navigationTrigger.eq(0).hasClass('nav-visible')) {
          ...
          navigationTrigger.eq(0).removeClass('nav-visible'); // setTimeout(() => {
